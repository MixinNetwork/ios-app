...
extension Web3OrderDAO {
    ...
    enum Offset: CustomDebugStringConvertible {
        ...
        case before(offset: TradeOrderViewModel, includesOffset: Bool)
        case after(offset: TradeOrderViewModel, includesOffset: Bool)
    ...
    func orders(
        ...
        if !filter.wallets.isEmpty {
            let walletIDs = filter.wallets.map(\.tradingWalletID)
        ...
        if let type = filter.type {
            switch type {
            case .swap:
                conditions.append("order_type = \(TradeOrder.OrderType.swap.rawValue)")
            case .limit:
                conditions.append("order_type = \(TradeOrder.OrderType.limit.rawValue)")
            }
        }
        ...
        let results: [TradeOrder] = db.select(with: query)
        if reverseResults {
...
extension Web3OrderDAO {
    ...
    func tradeOrderTokens(orders: [TradeOrder]) -> [String: TradeOrder.Token] {
