...
enum Pearl {
    ...
    struct DecodedTransaction {
        ...
        struct Input {
            ...
            fileprivate init(txid: String, vout: UInt32) {
                self.txid = txid
                self.vout = vout
                self.outputID = Web3Output.pearlOutputID(txid: txid, vout: vout)
            }
    ...
    static let spendingDust: Decimal = 100_000 * .satoshi
    static let changeDust = Decimal(PEARL_P2TR_DUST) * .satoshi
    static let privateKeyLength = PEARL_PRIVATE_KEY_LENGTH
    
    static func isValidAddress(address: String) -> Bool {
        address.withCString { address in
            pearl_is_valid_address(address)
        }
    }
    ...
    static func privateKey(mnemonics: String, path: String) throws -> Data {
        try mnemonics.withCString { mnemonics in
            try path.withCString { path in
                try withBitcoinDataPointer { bytes, count in
                    pearl_private_key_bytes_from_mnemonics(mnemonics, path, &bytes, &count)
                }
            }
        }
    }
    
    static func address(privateKey: Data) throws -> String {
        try privateKey.withUnsafeBytes { privateKey in
            try withBitcoinStringPointer { address in
                pearl_address(privateKey.baseAddress, privateKey.count, &address)
            }
        }
    }
    ...
    static func signedTransaction(
            ...
            try receiveAddress.withCString { receiveAddress in
                try privateKey.withUnsafeBytes { privateKey in
                    ...
                    let signedTransaction = try withBitcoinStringPointer { signedTransaction in
                        pearl_sign_taproot_transaction(
                            utxos.baseAddress,
                            utxos.count,
                            receiveAddress,
                            sendAmountInSatoshi,
        ...
        if change > 0 {
            ...
            changeOutput = Web3Output(
                id: Web3Output.pearlOutputID(txid: txid, vout: 1),
                assetID: AssetID.pearl,
    ...
    fileprivate static func withBitcoinDataPointer(
        ...
        guard count != -1 else {
            throw PearlError.invalidResult
        }
    ...
    struct TaprootFeeCalculator {
        ...
        struct Result: CustomDebugStringConvertible {
            ...
            var debugDescription: String {
                "<PearlFee transfer: \(transferAmount), fee: \(feeAmount), outputs: \(spendingOutputs.count)>"
            }
        ...
        func calculate(transferAmount: Decimal) throws(CalculateError) -> Result {
            ...
            for output in allOutputs {
                ...
                } else if utxoAmount < transferAmount + feeWithChange + Pearl.changeDust {
                    return Result(
                        transferAmount: transferAmount,
        ...
        private func vSize(
            ...
            let inputSize = Decimal(numberOfInputs) * 58
            let outputSize = Decimal(numberOfOutputs) * 43
