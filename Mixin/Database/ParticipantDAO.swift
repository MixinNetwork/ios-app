...
final class ParticipantDAO {
    ...
    """
    static let sqlQueryNotSentKeyParticipants = """
    \(sqlQueryColumns)
    ...
    SELECT u.user_id, u.full_name, u.identity_number, u.avatar_url, u.phone, u.is_verified, u.mute_until, u.app_id, u.relationship, u.created_at, a.description as appDescription, p.role
    FROM participants p
    ...
    func getSyncParticipantIds() -> [String] {
        return MixinDatabase.shared.getStringValues(column: Participant.Properties.userId.asColumnResult(), tableName: Participant.tableName, isDistinct: true, condition: Participant.Properties.status == ParticipantStatus.START.rawValue, inTransaction: false)
    }
