...
extension MobileNumberViewController: UITextFieldDelegate {
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        if newText != numericsInText, let parsedPhoneNumber = try? phoneNumberValidator.phoneNumberParser.parse(newText), let country = CountryCodeLibrary.shared.countries.first(where: { $0.callingCode == String(parsedPhoneNumber.countryCode) }) {
