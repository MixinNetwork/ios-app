import Foundation
import GRDB

public func ftsContent(
    messageId: String,
    category: String,
    content: String?,
    name: String?,
    caption: String?,
    children: [TranscriptMessage]? = nil
) -> String? {
    guard let category = MessageCategory(rawValue: category) else {
        return nil
    }
    switch category {
    case .PLAIN_DATA, .SIGNAL_DATA, .ENCRYPTED_DATA:
        return name
    case .SIGNAL_TEXT, .PLAIN_TEXT, .ENCRYPTED_TEXT, .SIGNAL_POST, .PLAIN_POST, .ENCRYPTED_POST:
        return content
    case .SIGNAL_IMAGE, .PLAIN_IMAGE, .ENCRYPTED_IMAGE:
        return caption
    case .SIGNAL_TRANSCRIPT, .PLAIN_TRANSCRIPT, .ENCRYPTED_TRANSCRIPT:
        let children = children ?? TranscriptMessageDAO.shared.childMessages(with: messageId)
        let ftsContents: [String] = children.compactMap { child in
            ftsContent(messageId: child.messageId,
                       category: child.category,
                       content: child.content,
                       name: child.mediaName,
                       caption: child.caption)
        }
        return ftsContents.joined(separator: " ")
    default:
        return nil
    }
}

extension DatabaseFunction {
    
    static let ftsContent = DatabaseFunction(
        "fts_content",
        argumentCount: 5,
        pure: true
    ) { (values) -> String? in
        guard
            values.count == 5,
            let messageId = values[0].storage.value as? String,
            let category = values[1].storage.value as? String
        else {
            return nil
        }
        let content = values[2].storage.value as? String
        let name = values[3].storage.value as? String
        let caption = values[4].storage.value as? String
        return MixinServices.ftsContent(
            messageId: messageId,
            category: category,
            content: content,
            name: name,
            caption: caption,
            children: nil
        )
    }
    
}
