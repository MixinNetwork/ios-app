...
struct TradeOrderViewModel {
    
    struct AssetChange {
        let token: TradeOrder.Token?
        let amount: String
    }
    ...
    let orderID: String
    let wallet: Wallet
    let state: UnknownableEnum<TradeOrder.State>
    let type: UnknownableEnum<TradeOrder.OrderType>
    
    let payAssetID: String
    let payToken: TradeOrder.Token?
    let paySymbol: String
    let paying: AssetChange
    
    let receiveAssetID: String
    let receiveToken: TradeOrder.Token?
    ...
    init(
        order: TradeOrder,
        wallet: Wallet,
        payToken: TradeOrder.Token?,
        receiveToken: TradeOrder.Token?,
    ) {
...
extension TradeOrderViewModel: Equatable {
