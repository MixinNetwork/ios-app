...
final class TradeOrderViewController: UITableViewController {
    
    private var viewModel: TradeOrderViewModel
    private var actions: [Action] = []
    private var infoRows: [InfoRow] = []
    private var loader: PendingTradeOrderLoader?
    
    init(viewModel: TradeOrderViewModel) {
    ...
    override func viewDidLoad() {
        ...
        tableView.register(R.nib.tradeOrderHeaderCell)
        ...
        tableView.register(R.nib.tradeOrderIDCell)
        ...
        case .created, .pending:
            if let type = viewModel.type.knownCase {
                loader = PendingTradeOrderLoader(
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case .header:
            let cell = tableView.dequeueReusableCell(withIdentifier: R.reuseIdentifier.trade_order_header, for: indexPath)!
        ...
        case .info:
            ...
            case .createdAt:
                ...
                return cell
            case .orderID:
                let cell = tableView.dequeueReusableCell(withIdentifier: R.reuseIdentifier.trade_order_id, for: indexPath)!
    ...
    @objc private func reloadOrderIfContains(_ notification: Notification) {
        guard let orders = notification.userInfo?[Web3OrderDAO.ordersUserInfoKey] as? [TradeOrder] else {
            return
        }
        guard let order = orders.first(where: { $0.orderID == viewModel.orderID }) else {
            return
        }
        let newViewModel = TradeOrderViewModel(
            order: order,
...
extension TradeOrderViewController {
    ...
    private enum InfoRow {
        ...
        case paid
        case receives
        case filling(TradeOrderViewModel.Filling)
        case price
        case wallet
        ...
        static func rows(viewModel: TradeOrderViewModel) -> [InfoRow] {
    ...
    private struct OneSideStablecoinPair {
        ...
        init?(order: TradeOrderViewModel) {
