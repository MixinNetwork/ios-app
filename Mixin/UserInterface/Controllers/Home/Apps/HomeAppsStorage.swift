            ...
            var items = coder.items.compactMap { item -> HomeAppItem? in
                ...
                case .app(let app):
                    if !existedIds.insert(app.id).inserted {
                        Log.general.error(category: "HomeAppsStorage", message: "A duplicated app is detected: \(app.id)")
                        return nil
                    }
                    return item
                case .folder(let folder):
                    let pages = folder.pages.compactMap { page -> [HomeApp]? in
                        let pageApps = page.compactMap { app -> HomeApp? in
                            if !existedIds.insert(app.id).inserted {
                                Log.general.error(category: "HomeAppsStorage", message: "A duplicated app in folder is detected: \(app.id)")
                                return nil
                            }
                            return app
                        }
