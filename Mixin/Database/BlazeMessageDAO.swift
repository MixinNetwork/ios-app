...
final class BlazeMessageDAO {
    ...
    func insertOrReplace(messageId: String, conversationId: String, data: String?, createdAt: String) -> Bool {
        guard let data = data?.data(using: .utf8) else {
            return false
        }
        return MixinDatabase.shared.insertOrReplace(objects: [MessageBlaze(messageId: messageId, conversationId: conversationId, message: data, createdAt: createdAt)])
    }
    ...
    func getBlazeMessageData(conversationId: String, limit: Int) -> [BlazeMessageData] {
        return MixinDatabase.shared.getCodables(on: [MessageBlaze.Properties.message], fromTable: MessageBlaze.tableName, condition: MessageBlaze.Properties.conversationId == conversationId, orderBy: [MessageBlaze.Properties.createdAt.asOrder(by: .ascending)], limit: limit) { (rows) -> [BlazeMessageData] in
            var result = [BlazeMessageData]()
