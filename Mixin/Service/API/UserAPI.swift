...
final class UserAPI: UserSessionAPI {
    ...
    func codes(codeId: String, completion: @escaping (MixinAPI.Result<QRCodeResponse>) -> Void) {
        request(method: .get, url: url.codes(codeId: codeId), completion: completion)
    }

    @discardableResult
    func showUser(userId: String, completion: @escaping (MixinAPI.Result<UserResponse>) -> Void) -> Request? {
    ...
    @discardableResult
    func showUsers(userIds: [String], completion: @escaping (MixinAPI.Result<[UserResponse]>) -> Void) -> Request? {
    ...
    @discardableResult
    func search(keyword: String, completion: @escaping (MixinAPI.Result<UserResponse>) -> Void) -> Request? {
        return request(method: .get, url: url.search(keyword: keyword), completion: completion)
    }

    func search(keyword: String) -> MixinAPI.Result<UserResponse> {
        return request(method: .get, url: url.search(keyword: keyword))
    }

    func addFriend(userId: String, full_name: String, completion: @escaping (MixinAPI.Result<UserResponse>) -> Void) {
    ...
    func removeFriend(userId: String, completion: @escaping (MixinAPI.Result<UserResponse>) -> Void) {
        let relationshipRequest = RelationshipRequest(user_id: userId, full_name: nil, action: .REMOVE)
        request(method: .post, url: url.relationships, parameters: relationshipRequest.toParameters(), encoding: EncodableParameterEncoding<RelationshipRequest>(), completion: completion)
    }

    func remarkFriend(userId: String, full_name: String, completion: @escaping (MixinAPI.Result<UserResponse>) -> Void) {
        let relationshipRequest = RelationshipRequest(user_id: userId, full_name: full_name, action: .UPDATE)
        request(method: .post, url: url.relationships, parameters: relationshipRequest.toParameters(), encoding: EncodableParameterEncoding<RelationshipRequest>(), completion: completion)
    }

    func blockUser(userId: String, completion: @escaping (MixinAPI.Result<UserResponse>) -> Void) {
        let relationshipRequest = RelationshipRequest(user_id: userId, full_name: nil, action: .BLOCK)
        request(method: .post, url: url.relationships, parameters: relationshipRequest.toParameters(), encoding: EncodableParameterEncoding<RelationshipRequest>(), completion: completion)
    }

    func blockUser(userId: String) -> MixinAPI.Result<UserResponse> {
        let relationshipRequest = RelationshipRequest(user_id: userId, full_name: nil, action: .BLOCK)
    ...
    func reportUser(userId: String, completion: @escaping (MixinAPI.Result<UserResponse>) -> Void) {
        let relationshipRequest = RelationshipRequest(user_id: userId, full_name: nil, action: .BLOCK)
    ...
    func reportUser(userId: String) -> MixinAPI.Result<UserResponse> {
        let relationshipRequest = RelationshipRequest(user_id: userId, full_name: nil, action: .BLOCK)
    ...
    func unblockUser(userId: String, completion: @escaping (MixinAPI.Result<UserResponse>) -> Void) {
        let relationshipRequest = RelationshipRequest(user_id: userId, full_name: nil, action: .UNBLOCK)
        request(method: .post, url: url.relationships, parameters: relationshipRequest.toParameters(), encoding: EncodableParameterEncoding<RelationshipRequest>(), completion: completion)
    }

    func getFavoriteApps(ofUserWith id: String, completion: @escaping (MixinAPI.Result<[FavoriteApp]>) -> Void) {
        request(method: .get, url: url.getFavorite(userId: id), completion: completion)
    }

    func setFavoriteApp(id: String, completion: @escaping (MixinAPI.Result<FavoriteApp>) -> Void) {
        request(method: .post, url: url.setFavorite(appId: id), completion: completion)
    }

    func unfavoriteApp(id: String, completion: @escaping (MixinAPI.Result<Empty>) -> Void) {
