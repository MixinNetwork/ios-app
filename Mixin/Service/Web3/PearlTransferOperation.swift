...
class PearlTransferOperation: Web3TransferOperation {
    ...
    static let assetID = AssetID.pearl
    ...
    override func start(pin: String) async throws {
        ...
        do {
            Logger.web3.info(category: "PearlTransfer", message: "Start")
            let privateKey = try await wallet.pearlPrivateKey(pin: pin, address: fromAddress)
            Logger.web3.info(category: "PearlTransfer", message: "Spend outputs: \(spendingOutputs)")
            signedTransaction = try Pearl.signedTransaction(
            ...
            if let changeOutput = signedTransaction.changeOutput {
                Logger.web3.info(category: "PearlTransfer", message: "Change: \(changeOutput)")
            }
        } catch {
            Logger.web3.error(category: "PearlTransfer", message: "Failed to sign: \(error)")
            await MainActor.run {
        ...
        do {
            Logger.web3.info(category: "PearlTransfer", message: "Will send tx")
            ...
            Logger.web3.info(category: "PearlTransfer", message: "Tx sent, hash: \(hash)")
            ...
            Logger.web3.info(category: "PearlTransfer", message: "Tx saved")
            await MainActor.run {
                self.state = .success
                self.hasTransactionSent = true
            }
        } catch {
            Logger.web3.error(category: "PearlTransfer", message: "Failed to send: \(error)")
            await MainActor.run {
...
final class PearlTransferToAddressOperation: PearlTransferOperation {
...
class PearlRBFOperation: PearlTransferOperation {
...
final class PearlSpeedUpOperation: PearlRBFOperation {
...
final class PearlCancelOperation: PearlRBFOperation {
    ...
    override func start(pin: String) async throws {
        ...
        do {
            ...
            let privateKey = try await wallet.pearlPrivateKey(pin: pin, address: fromAddress)
            Logger.web3.info(category: "PearlCancel", message: "Cancel inputs: \(spendingOutputs)")
            signedTransaction = try Pearl.signedTransaction(
                outputs: spendingOutputs,
                sendAddress: fromAddress.destination,
                sendAmount: sendAmount,
            ...
            Logger.web3.error(category: "PearlCancel", message: "Failed to sign: \(error)")
            await MainActor.run {
                self.state = .signingFailed(error)
            }
            return
        ...
        do {
            Logger.web3.info(category: "PearlCancel", message: "Will send tx")
            let rawTransaction = try await RouteAPI
                .postTransaction(
                    chainID: ChainID.pearl,
                    from: fromAddress.destination,
                ...
                .replacingNonce(with: feeRate)
            Logger.web3.info(category: "PearlCancel", message: "Tx sent, hash: \(rawTransaction.hash)")
            let pendingTransaction = Web3Transaction(
                rawTransaction: rawTransaction,
                fee: fee,
                myAddress: fromAddress.destination
            ...
            let receivingOutput = Web3Output(
                id: Web3Output.pearlOutputID(txid: rawTransaction.hash, vout: 0),
                assetID: token.assetID,
            ...
            Logger.web3.info(category: "PearlCancel", message: "Tx saved")
            await MainActor.run {
                self.state = .success
                self.hasTransactionSent = true
            }
        } catch {
            Logger.web3.error(category: "PearlCancel", message: "Failed to send: \(error)")
            await MainActor.run {
                self.state = .sendingFailed(error)
            }
        }
