...
extension HomeAppsItemManager {
        ...
        do {
            if let parsedDict = try JSONSerialization.jsonObject(with: jsonData, options: []) as? JSONDictionary,
               ...
               let pagesJSONArray = parsedDict["pages"] as? [JSONArray] {
                pinnedItems = appItems(from: pinnedJSONArray, allAppIds: allAppIds, embeddedApps: embeddedApps, appUsers: appUsers)
                candidateItems = pagesJSONArray.compactMap({ jsonArray -> [AppItem]? in
                    let page = appItems(from: jsonArray, allAppIds: allAppIds, embeddedApps: embeddedApps, appUsers: appUsers)
    ...
    private func appItems(from jsonArray: JSONArray, allAppIds: [String], embeddedApps: [EmbeddedApp], appUsers: [User]) -> [AppItem] {
        return jsonArray.compactMap { item -> AppItem? in
            ...
            case .app:
                ...
                if let embeddedApp = embeddedApps.first(where: { $0.id == id }) {
                    return AppModel(id: id, app: .embedded(embeddedApp))
                } else if let appUser = appUsers.first(where: { $0.appId == id }) {
