...
let uuidToToken = DatabaseFunction("utot", argumentCount: 1, pure: true) { (values) -> String? in
...
let tokenToUUID = DatabaseFunction("ttou", argumentCount: 1, pure: true) { (values) -> String? in
    switch values.first?.storage {
    case let .string(text):
