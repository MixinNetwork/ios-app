...
struct MixinTests {
    ...
    @Test func testApplePayProvisioningStartMessage() throws {
        let requestID = "019f45d4-15ee-7598-a940-41c695b25a1a"
        let message = WebViewMessageHandler.applePayProvisioningStartMessage(from: """
        {
          "requestId": "\(requestID)",
          "primaryAccountSuffix": "4242",
          "primaryAccountIdentifier": "card-4242",
          "localizedDescription": "Mixin Card",
          "paymentNetwork": "visa"
        }
        """)

        guard case let .startApplePayProvisioning(request) = message else {
            Issue.record("Expected an Apple Pay provisioning start message")
            return
        }
        #expect(request.requestID.uuidString.lowercased() == requestID)
        #expect(request.cardholderName == nil)
        #expect(request.primaryAccountSuffix == "4242")
        #expect(request.primaryAccountIdentifier == "card-4242")
        #expect(request.localizedDescription == "Mixin Card")
        #expect(request.paymentNetwork == "visa")
    }

    @Test func testApplePayProvisioningCompletionMessages() throws {
        let requestID = "019f45d4-15ee-7598-a940-41c695b25a1a"
        let success = WebViewMessageHandler.applePayProvisioningCompletionMessage(from: """
        {
          "requestId": "\(requestID)",
          "activationData": "AQ==",
          "encryptedPassData": "Ag==",
          "ephemeralPublicKey": "Aw=="
        }
        """)
        guard case let .completeApplePayProvisioning(.payload(id, activation, encrypted, key)) = success else {
            Issue.record("Expected an Apple Pay provisioning payload")
            return
        }
        #expect(id.uuidString.lowercased() == requestID)
        #expect(activation == Data([1]))
        #expect(encrypted == Data([2]))
        #expect(key == Data([3]))

        let failure = WebViewMessageHandler.applePayProvisioningCompletionMessage(from: """
        {"requestId":"\(requestID)","error":"issuer_unavailable"}
        """)
        guard case let .completeApplePayProvisioning(.failure(id, error)) = failure else {
            Issue.record("Expected an Apple Pay provisioning failure")
            return
        }
        #expect(id.uuidString.lowercased() == requestID)
        #expect(error == "issuer_unavailable")

        if case .some = WebViewMessageHandler.applePayProvisioningStartMessage(from: [:]) {
            Issue.record("Expected dictionary transport to be rejected")
        }
        if case .some = WebViewMessageHandler.applePayProvisioningCompletionMessage(from: """
        {"requestId":"\(requestID)","activationData":"!","encryptedPassData":"Ag==","ephemeralPublicKey":"Aw=="}
        """) {
            Issue.record("Expected invalid base64 to be rejected")
        }
    }
    ...
    @Test func testUserIDHash() {
        ...
        let hashes = userIDs.map { userID in
